...
const Wardrobe: React.FC<WardrobeProps> = ({
  ...
  // Handle item deletion with user authentication
  const handleDeleteItem = async (itemId: string, itemName: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setDeleteError(null);
    
    if (!user?.id) {
      setDeleteError('You must be logged in to delete items');
      return;
    }
  ...
  // Safe image URL function for Supabase items
  const getSafeImageUrl = (item: any): string => {
    ...
    // Use the getImageUrl function from the hook
    if (item?.image) {
  ...
  // Show loading message if no user
  if (!user) {
    return (
      <>
        <Header
        ...
        <div style={styles.container}>
          <div style={styles.emptyState}>
            <h3>Please log in to view your wardrobe</h3>
            <button
    ...
    <>
      ...
      {selectedItemForDetails && (
        <ItemDetailsModal
          ...
          getImageUrl={(imagePath: string) => getSafeImageUrl({ image: imagePath })}
        />
      )}
